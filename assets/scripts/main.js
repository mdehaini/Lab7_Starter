   ...
   */
   if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
      navigator.serviceWorker.register('https://mdehaini.github.io/Lab7_Starter/sw.js').then(function(registration) {
...
function bindPopstate() {
   ...
   */
   window.addEventListener('popstate', e => {
      if (e.path[0].nodeName == 'A') return;
      if(e.state == null){ //added statement to deal with edgecase
        router.navigate('home', true); 
      }
      else{
        router.navigate(e.state.state_page, true);
      }
  });
}
